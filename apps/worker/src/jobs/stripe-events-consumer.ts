...
async function handlePaymentSuccess(orderId: string, stripeEventId?: string) {
  try {
    // Call RPC function to confirm payment (handles idempotency, allocation conversion, event emission)
    const { data: result, error: rpcError } = await supabase.rpc("confirm_order_payment", {
      order_id: orderId,
    ...
    console.log(`Order ${orderId} payment confirmed:`, result);
  } catch (error) {
...
async function handlePaymentFailure(orderId: string) {
  try {
    ...
    console.log(`Order ${orderId} payment failed, reservations released:`, result);
  } catch (error) {
