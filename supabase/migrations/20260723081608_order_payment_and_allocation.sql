...
-- Known bug preserved here as originally applied, fixed by a later
-- migration (20260724160000_fix_checkout_payment_confirmation.sql) rather
-- than edited in place: this file's confirm_order_payment() and
-- release_failed_order_reservations() reference cart_lines.reservation_id,
-- but the actual column (migration 20260723070153_carts.sql) is
-- inventory_reservation_id, and confirm_order_payment() calls
-- allocate_order_inventory() with 3 positional args
-- (reservation_id, sellable_sku_id, quantity) when its real signature
-- (migration 20260723060956_allocate_and_pick_inventory.sql) is
-- (p_reservation_id uuid, p_order_line_id uuid default null) -- 2 args,
-- and the second one means something different. Both bugs mean every call
-- to confirm_order_payment() throws before completing, i.e. payment
-- confirmation is broken today.
