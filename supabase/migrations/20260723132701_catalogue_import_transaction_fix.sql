...
-- too -- wiping the run row before the exception handler could reference it,
-- causing a foreign key violation in catalogue_import_errors. Splitting the
-- run insert into an outer block (which survives) and the risky card
-- processing into a nested block (whose failure only rolls back its own
-- work) fixes this. The function now also returns a status instead of
-- re-raising, so failure bookkeeping actually commits with the transaction
-- instead of being rolled back by a propagated exception.
