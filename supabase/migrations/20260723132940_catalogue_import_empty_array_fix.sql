...
begin
  ...
  values (
    ...
    p_card->>'toughness',
    p_card->>'loyalty',
    coalesce((select array_agg(val) from jsonb_array_elements_text(p_card->'colors') as val), array[]::text[]),
    coalesce((select array_agg(val) from jsonb_array_elements_text(p_card->'colorIdentity') as val), array[]::text[])
  )
  ...
  values (
    ...
    p_card->>'number',
    p_card->>'rarity',
    coalesce((select array_agg(val) from jsonb_array_elements_text(p_card->'finishes') as val), array[]::text[]),
    p_card->>'frameVersion',
    p_card->>'borderColor',
    p_card->>'flavorText',
